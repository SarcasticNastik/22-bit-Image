...
weights_init, bias_init, Conv= T.nn.init.kaiming_normal_, T.nn.init.zeros_, partial(nnt.Conv2d, weights_init=weights_init, bias_init=bias_init)
...
class _PCEncoderMethods:
    def _project_old(self, img_feats, xs, ys):
        ...
        for i in range(list(img_feats.shape)[0]):
            ...
            weights_q11 = ((x2 - x) * (y2 - y)).unsqueeze(0)
            q11 *= weights_q11

            q21 = img_feat[..., x2.long(), y1.long()].cuda()
            weights_q21 = ((x - x1) * (y2 - y)).unsqueeze(0)
            q21 *= weights_q21

            q12 = img_feat[..., x1.long(), y2.long()].cuda()
            weights_q12 = ((x2 - x) * (y - y1)).unsqueeze(0)
            q12 *= weights_q12

            q22 = img_feat[..., x2.long(), y2.long()].cuda()
            weights_q22 = ((x - x1) * (y - y1)).unsqueeze(0)
            q22 *= weights_q22
    ...
    def get_projection(self, img_feat, pc):
        ...
        h, w = T.clamp(248. * Y / Z + 111.5, 0., 223.), T.clamp(248. * -X / Z + 111.5, 0., 223.)
        x, y = (h / (223. / (h_ - 1.))).requires_grad_(False), (w / (223. / (w_ - 1.))).requires_grad_(False)
