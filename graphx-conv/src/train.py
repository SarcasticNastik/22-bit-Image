...
def train_valid(data_root, name, img_enc, pc_enc, pc_dec, optimizer, scheduler, adain=True, projection=True,
    ...
    if optimizer is not None:
        solver = optimizer(net.trainable, lr, weight_decay=weight_decay)
    else:
        solver = T.optim.Adam(net.trainable, 1e-4, weight_decay=0)

    if scheduler is None:
        scheduler = None
    else:
        scheduler = scheduler(solver, milestones=milestones, gamma=gamma)
    ...
    mon.model_name, mon.print_freq, mon.num_iters  = name, print_freq, len(train_data) // bs
